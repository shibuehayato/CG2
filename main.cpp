...
Microsoft::WRL::ComPtr<ID3D12Resource>CreateTextureResource(Microsoft::WRL::ComPtr<ID3D12Device> device, const DirectX::TexMetadata& metadata)
{
	...
	resourceDesc.DepthOrArraySize = UINT16(metadata.arraySize); // 奥行き or 配列
...
void UploadTextureDate(Microsoft::WRL::ComPtr<ID3D12Resource> texture, const DirectX::ScratchImage& mipImages)
{
	// Meta情報を取得
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	D3D12_COMMAND_QUEUE_DESC commanQueueDesc{};
	hr = device->CreateCommandQueue(&commanQueueDesc,
	...
	rootParameters[1].DescriptorTable.pDescriptorRanges = descriptorRangeForInstancing;  // Tableの中身の配列を使う
	...
	*wvpData = MakeIdentity4x4();
	...
	float color[4] = { materialData->position.x, materialData->position.y, materialData->position.z ,materialData->position.w};
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			*wvpData = worldMatrix;
			...
			for (uint32_t index = 0; index < kNumInstance; ++index)
			{
				particles[index].transform.translate = Add(particles[index].transform.translate,MulVector3AndFloat(particles[index].velocity,kDeltaTime));
			...
			ImGui::End();
			materialData->position.x = color[0];
			materialData->position.y = color[1];
			materialData->position.z = color[2];
			materialData->position.w = color[3];
